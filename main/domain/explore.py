...
def _move_ap_cost(actor, halved: bool = False) -> int:
    ...
    armor_multiplier = actor.armor_penalty()["speed_multiplier"]
    s = actor.effective_speed * armor_multiplier
    if halved:
        s /= 2.0
...
class ExploreMixin:
    ...
    def _check_traps(self, creature, pos: tuple[int, int]) -> None:
        ...
        if self.emit_log:
            self.emit_log(f"{creature.name} 踩中了地精陷阱，被木刺戳到了小腿")
        ...
        for trap in self.traps:
            ...
            if not self.spot_memo.get(pos, False):
                ...
                if roll >= trap.dc:
                    ...
                    if self.emit_log:
                        self.emit_log(f"{observer.name} 发现了一处埋藏的陷阱。")
            elif not trap.sight_log_fired:
                trap.sight_log_fired = True
                if self.emit_log:
                    self.emit_log("一处埋藏的陷阱")
