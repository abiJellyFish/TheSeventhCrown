...
def apply_final_damage(target, amount: int, damage_type: str,
                       critical: bool = False) -> int:
    """统一执行伤害类型修正和最终伤害应用。"""
    normalized_type = normalize_damage_type(damage_type)
    modified = apply_damage_type_modifiers(amount, normalized_type, target)
    return apply_damage(target, modified, normalized_type, critical=critical)
...
def hit_check(attacker: Entity, defender: Entity, weapon: Item | None = None,
              ...
              guaranteed: bool = False, mod: int | None = None,
              expertise_adjust: int = 0) -> tuple[bool, int]:
    ...
    Args:
        ...
        expertise_adjust: 专精项在命中比较时选择的修正，范围为 -5~+5。

    Returns:
        (是否命中, D20 自然结果)
    """
    if not -5 <= expertise_adjust <= 5:
        raise ValueError("专精项修正必须在 -5 到 +5 之间")
    natural_roll = chosen_roll if chosen_roll is not None else _roll_auto(adv)
    roll = natural_roll + expertise_adjust
    if mod is None:
    ...
    if guaranteed:
        ...
        adjusted_roll = min(20, max(2, ac - mod))
        return True, adjusted_roll
...
def compute_attack_adv(attacker: Entity, defender: Entity, weapon: Item,
    ...
    if weapon is not None:
        ...
        armor_penalty = attacker.armor_penalty()
        if attack_stat in ("str", "dex"):
            if attack_stat == "str" and armor_penalty["str_disadvantage"]:
                adv -= 1
            if attack_stat == "dex" and armor_penalty["dex_disadvantage"]:
                adv -= 1
    ...
    if defender.has_status("昏迷"):
        adv += 1
        if attacker_pos is not None and defender_pos is not None:
            adjacent = max(abs(attacker_pos[0] - defender_pos[0]),
                           abs(attacker_pos[1] - defender_pos[1])) <= 1
    ...
    if hidden:
        adv += 1
    # 目标在轻度遮蔽格 → 远程命中劣势（即使目标未隐匿/已被发现）
    if light_cover and weapon.weapon_type == "ranged":
        adv -= 1
...
def _load_hit_locations() -> dict[str, list[tuple[str, int]]]:
    ...
    data = _REPOSITORY.load_json("hit_locations.json")
    if data:
...
def _load_damage_modifiers() -> dict[str, dict[str, str | None]]:
    ...
    data = _REPOSITORY.load_json("damage_modifiers.json")
    if data:
        return data
    return {
...
def apply_damage_type_modifiers(damage: int, damage_type: str,
    ...
    结算顺序: 免疫 → 抗性(÷2) → 易伤(×2)
    """
    traits = getattr(defender, "traits", []) or []
    for trait in traits:
    ...
    if hasattr(defender, "has_status") and defender.has_status("潮湿"):
        if damage_type == "fire":
...
def resolve_attack(
    ...
    *,
    ...
    light_cover: bool = False,
    award_weapon_experience: bool = True,
) -> dict:
    """完整一次攻击结算（含掩体检查）。
    ...
    Args:
        ...
        guaranteed: 必定命中（如魔法飞弹）。命中检定未通过时改为刚好成功的点数，
            必定命中（见 hit_check）。
        award_weapon_experience: 是否为本次攻击发放武器经验；战技复用结算时关闭。
    ...
    """
    if award_weapon_experience and hit is None:
        attacker.grant_weapon_exp(weapon.category, 0.01)
    ...
    if hit is None:
        ...
        adjacent_comatose = (
            defender.has_status("昏迷")
            and attacker_pos is not None
            and target_pos is not None
            and max(abs(attacker_pos[0] - target_pos[0]),
                    abs(attacker_pos[1] - target_pos[1])) <= 1
        )
        if adjacent_comatose:
            hit, roll = True, 20
        else:
    ...
    modified_damage = apply_damage_type_modifiers(damage, dmg_type, defender)

    # 应用伤害（统一走 apply_damage；实体扣 HP，物品扣耐久）
    # 击晕模式（阶段9）：近战 + 非致命 + 未超 2×max_hp + 会致死 → HP=1 + 昏迷
    if (isinstance(defender, Entity) and nonlethal
            and weapon.weapon_type == "melee"
            and defender.hp - modified_damage < 1
            and modified_damage < 2 * defender.max_hp):
        damage = modified_damage
        defender.hp = 1
        defender._interrupted = True
        defender.add_status("昏迷")
    else:
        damage = apply_final_damage(defender, damage, dmg_type, critical=critical)
    ...
    from domain.classes import CLASS_EXP_BY_WEAPON_CATEGORY
    category = weapon.category if weapon is not None else "simple"
    class_exp = CLASS_EXP_BY_WEAPON_CATEGORY.get(category or "simple", 0.01)
    result["class_leveled"] = attacker.grant_route_exp("fighter", class_exp)
    attacker.grant_attribute_exp("str", 0.01)
    return result
...
class AutoHitAttack:
    ...
    def resolve(self, attacker: Entity | None, defender) -> list[int]:
        ...
        for _ in range(self.missiles):
            ...
            if defender:
                if attacker is not None and isinstance(defender, Entity):
        ...
        if defender:
            total = sum(results)
            apply_final_damage(defender, total, self.damage_type)
        return results
