...
class ActionResolverMixin:
    ...
    def _do_roll(self, actor, target=None, target_pos=None) -> bool:
        ...
        if self.emit_log:
            if was_burning:
                self.emit_log(f"{actor.name} 在地上打了个滚，扑灭了火焰")
            else:
                self.emit_log(f"{actor.name} 在地上打了个滚")
        return True
    ...
    def _do_dodge(self, actor, target=None, target_pos=None) -> bool:
        ...
        if self.emit_log:
            self.emit_log(f"{actor.name} 进入回避状态")
        return True
    ...
    def _do_hide(self, actor, target=None, target_pos=None) -> bool:
        ...
        if actor.has_status("hiding"):
            if self.emit_log:
                self.emit_log(f"{actor.name} 已经处于躲藏状态")
            return False
        ...
        for c, (ec, er) in self.entities:
            ...
            if self._observer_can_see(c, actor_pos):
                if self.emit_log:
                    self.emit_log(f"{c.name} 发现了 {actor.name}")
        return True
    ...
    def _do_stand(self, actor, target=None, target_pos=None) -> bool:
        ...
        if self.in_combat and actor.ap < cost:
            if self.emit_log:
                self.emit_log(f"{actor.name} AP 不足，无法起身")
            return False
        if prone:
        ...
        if self.emit_log:
            ...
            self.emit_log(f"{actor.name} {body} (消耗 {cost if self.in_combat else pendulum} {'AP' if self.in_combat else '钟摆'})")
        return True
    ...
    def _do_search(self, actor, target=None, target_pos=None) -> bool:
        ...
        for target_id, observers in list(self.hidden_from.items()):
            ...
            if self._passive_spot(actor, tgt):
                ...
                found_any = True
                if self.emit_log:
                    self.emit_log(f"{actor.name} 搜出了 {tgt.name}")
        # 线索/可调查对象智力检定（阶段5，D9）
        clue_result = self._investigate_clues(actor)
        found_any = found_any or clue_result
        if self.emit_log and not found_any:
            self.emit_log("你感到周围一切正常")
        return True
    ...
    def _investigate_clues(self, actor) -> bool:
        ...
        for trap in self.traps:
            ...
            if roll >= trap.dc:
                ...
                if self.emit_log:
                    self.emit_log(f"{actor.name} 发现了一处埋藏的陷阱。")
                result = True
        # 线索探查（智力检定）
        for clue in list(self.clues):
            ...
            if roll >= clue.dc:
                ...
                if self.emit_log:
                    self.emit_log(clue.egg_text or f"你仔细研究{clue.label}，发现了重要线索")
                result = True
        return result
    ...
    def _jump_charge(self, actor, cells: int) -> None:
        ...
        if self.in_combat:
            ...
            for _ in range(cells):
                self.clock.tick_move(actor.effective_speed)
    ...
    def _jump_execute(self, actor, actor_pos, target_pos, stat_check, dist) -> bool:
        ...
        for i in range(count - 1):
            ...
            obstacle = obstacle_at(mid, self.entities, self.ground_items)
    ...
    def _shove_check(self, actor, target, target_pos=None) -> str:
        ...
        if max(abs(tgt_pos[0] - actor_pos[0]),
            ...
            return "invalid"
        # 体型限制：目标体型最多高一级（D11）
        from domain.entity import size_rank
    ...
    def _apply_shove(self, actor, target, target_pos=None, result="prone") -> bool:
        ...
        if result == "push":
            ...
            target.add_status("prone", duration=None)
            if self.emit_log:
                self.emit_log(f"{target.name} 身后是障碍，{actor.name} 将其撞倒在地")
            return True
        # "prone"
        target.add_status("prone", duration=None)
        if self.emit_log:
            self.emit_log(f"{actor.name} 一把将 {target.name} 推倒在地")
        return True
    ...
    def _do_shove(self, actor, target=None, target_pos=None,
        ...
        if st == "too_big":
            if self.emit_log:
                self.emit_log(f"{target.name} 体型太大，无法被推撞")
            return False
        if st == "fail":
            if self.emit_log:
                self.emit_log(f"{actor.name} 推了 {target.name} 一下，但没能撼动对方")
    ...
    def _do_help(self, actor, target=None, target_pos=None) -> bool:
        ...
        if target.has_status("濒死"):
            ...
            if roll >= 10:
                ...
                if self.emit_log:
                    self.emit_log(f"{actor.name} 尝试急救 {target.name}，但失败了")
            return True
        if target.has_status("prone"):
            target.remove_status("prone")
            if self.emit_log:
                self.emit_log(f"{actor.name} 扶起了 {target.name}")
        else:
...
def _load_special_actions() -> list:
    ...
    _SPECIAL_ACTIONS_CACHE = _REPOSITORY.load_json("maneuvers.json").get(
        "special_actions", []
    )
    return _SPECIAL_ACTIONS_CACHE
