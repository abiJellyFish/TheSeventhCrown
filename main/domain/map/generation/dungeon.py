"""地下城地图生成 —— BSP 算法生成房间 + 走廊。"""

import random
from domain.game_state import GameState
from domain.grid import Grid
from domain.movement import Terrain
from domain.item_actions import place_on_ground
from domain.trade import load_item


def build_dungeon(state: GameState, loader) -> None:
    """BSP 生成地下城 (30×20)。"""
    w, h = 30, 20
    state.current_map = "地下城"
    state.in_dungeon = True
    state.map = Grid[Terrain](w, h, Terrain.BARREN)
    state.entities = []
    state.ground_items = []
    state.map_exits = []
    state.location_map = {}  # 地下城全部标记为地下城1层
    state.burning_surfaces.clear()
    state.wet_surfaces.clear()
    state.light_sources.clear()
    state.invalidate_spatial_cache()

    # 挖掘 3-5 个房间 + 走廊
    rooms = []
    for _ in range(random.randint(3, 5)):
        rw, rh = random.randint(4, 8), random.randint(3, 6)
        rx = random.randint(1, w - rw - 1)
        ry = random.randint(1, h - rh - 1)
        rooms.append((rx, ry, rw, rh))
        for x in range(rx, rx + rw):
            for y in range(ry, ry + rh):
                state.map[x, y] = Terrain.FLOOR

    # 连接走廊
    for i in range(len(rooms) - 1):
        x1 = rooms[i][0] + rooms[i][2] // 2
        y1 = rooms[i][1] + rooms[i][3] // 2
        x2 = rooms[i + 1][0] + rooms[i + 1][2] // 2
        y2 = rooms[i + 1][1] + rooms[i + 1][3] // 2
        for x in range(min(x1, x2), max(x1, x2) + 1):
            state.map[x, y1] = Terrain.FLOOR
        for y in range(min(y1, y2), max(y1, y2) + 1):
            state.map[x2, y] = Terrain.FLOOR

    # 未挖掘区域由墙物品表达，地表始终保持基础地面。
    for x in range(w):
        for y in range(h):
            if state.map[x, y] is not Terrain.FLOOR:
                wall = load_item("墙壁")
                if wall is None:
                    raise RuntimeError("缺少地图物品：墙壁")
                place_on_ground(state.ground_items, wall, x, y)

    # 入口和出口（返回地面的楼梯）
    first_room = rooms[0]
    state.map[first_room[0] + 1, first_room[1]] = Terrain.STAIRS_UP  # 出口
    state.controlled_entity_pos = (first_room[0] + 2, first_room[1] + 1)

    # 红宝石在最后一个房间
    last_room = rooms[-1]
    state.map[last_room[0] + last_room[2] // 2, last_room[1] + last_room[3] // 2] = Terrain.FLOOR

    # 骷髅兵
    skeleton_positions = []
    for _ in range(4):
        r = random.choice(rooms[1:])
        sx = r[0] + random.randint(1, r[2] - 1)
        sy = r[1] + random.randint(1, r[3] - 1)
        if (sx, sy) not in skeleton_positions:
            skeleton_positions.append((sx, sy))
            sk = loader.load_entity("骷髅")
            if sk:
                sk.template_name = "骷髅"
                state.add_entity(sk, (sx, sy))

    # 任务物品：只从无全身障碍且未被实体占用的地面格随机投放一次。
    occupied = {pos for _, pos in state.entities}
    candidates = [
        (x, y) for x in range(w) for y in range(h)
        if state.map[x, y] == Terrain.FLOOR
        and (x, y) not in occupied
        and (x, y) != (first_room[0] + 1, first_room[1])
    ]
    if not candidates:
        raise RuntimeError("地下城没有可投放麦斯神像红宝石的合法格子")
    ruby = load_item("麦斯神像的红宝石")
    if ruby is None:
        raise RuntimeError("缺少任务物品：麦斯神像的红宝石")
    place_on_ground(state.ground_items, ruby, *random.choice(candidates))

    state.seed_campfires()
