...
class StealthMixin:
    ...
    def _cover_level(self, pos: tuple[int, int], observer_pos: tuple[int, int] | None = None) -> str:
        ...
        重度遮蔽 = 黑暗（DARK）光照 或 不在观察者FOV内 或 被墙体完全阻断。
        轻度遮蔽 = 灌木/矮墙 或 雾气格 或 微光（DIM）光照。
        """
        ...
        t = self.map[col, row]
        lm = self.light_map
        # 黑暗光照 → 重度遮蔽（黑暗是全地图的重度遮蔽）
        if lm is not None and lm[pos] == LightLevel.DARK:
            return "heavy"
        # 雾气格 → 轻度遮蔽
        if pos in self.fog_surfaces:
            return "light"
        # 障碍遮蔽统一读取实体或地面物品自身组件。
        if is_light_cover(self, pos):
            return "light"
        # 微光光照 → 轻度遮蔽
    ...
    def _has_stealth_qualification(self, target: Entity) -> bool:
        """目标是否具备隐匿资格：倒地/躲藏状态，或小型/微型生物自动躲藏。"""
        if target.has_status("prone") or target.has_status("hiding"):
            return True
    ...
    def _break_stealth_in_view(self, actor: Entity, force_targets: tuple = ()) -> None:
        ...
        if removed and self.emit_log:
            self.emit_log(f"{actor.name} 的动作暴露了自己（{', '.join(removed)} 看见了）")
    ...
    def _on_fov_recompute(self, observer: Entity) -> None:
        ...
        for target_id in new_entries:
            ...
            if not self._passive_spot(observer, target):
                ...
                if self.emit_log:
                    self.emit_log(f"{observer.name} 注意到了 {target.name} 的动静")
        # 更新快照
