...
from domain.obstacle import ObstacleType, normalize_obstacle_type


@dataclass
class ObstacleComponent:
    """实体障碍能力；不承载耐久或堆叠状态。"""
    obstacle_type: ObstacleType = ObstacleType.THREE_QUARTER
    block_value: int = 0
...
class ClassComponent:
    ...
    class_experience: dict[str, float] = field(default_factory=dict)
    titles: list[str] = field(default_factory=list)
    domain_talents: list[str] = field(default_factory=list)
    domain_experience: dict[str, float] = field(default_factory=dict)
    weapon_experience: dict[str, float] = field(default_factory=dict)
    attribute_experience: dict[str, float] = field(default_factory=dict)
...
class InventoryComponent:
    ...
    equipment: dict = field(default_factory=lambda: {
        ...
        "left_hand": None, "right_hand": None, "spellbook": None,
    })
