...
from presentation.textual.screens.title_main import TitleScreen
...
class GameScreen(InventoryMixin, InteractMixin, TargetingMixin, NpcRunnerMixin, CommandMixin, KeybindMixin, Screen):
    ...
    * { margin: 0; padding: 0; overflow: hidden; }
    ...
    BINDINGS = [
        ...
        Binding("slash", "rotate_target", "", priority=False),
    ]

    # ── 视图注册表：每个视图声明支持的按键和输入命令 ──

    _EXPLORE_KEYS = {
        "0", "N", "g", "G", "r", "R", "comma", "F", "A", "S", "5",
    }
    ...
    def _add_visible_action_log(self, message: str, position=None) -> None:
        ...
        if position is None or position == self._state.controlled_entity_pos:
            self._act_log.add(message)
            return
        if position in self._state.fov_cache:
            self._act_log.add(message)
    ...
    def _cast_spell(self, spell: dict, target: "Entity | None | list", target_pos: tuple[int, int] | None = None) -> None:
        ...
        if effect_type == "wet_tile":
            ...
            if spell.get("effect", {}).get("ignite"):
                ...
                for cell in cells:
                    terrain = self._state.map[cell[0], cell[1]]
                    if terrain in FLAMMABLE:
                        self._state.burning_surfaces[cell] = BurningSurface(
                            fuel=FLAMMABLE[terrain]
                        )
                        self._state.register_light(cell, 1, LightLevel.BRIGHT)
            # 伤害型法术 → 检查态度反应（多目标逐个）
    ...
    def _enter_dungeon(self) -> None:
        ...
        self._state.world_state = {
            ...
            "controlled_entity_pos": self._state.controlled_entity_pos, "current_map": self._state.current_map,
        ...
        if not any(c is p for c, _ in self._state.entities):
            self._state.entities.append((p, self._state.controlled_entity_pos))
    ...
    def _exit_dungeon(self) -> None:
        ...
        if ws:
            ...
            self._state.ground_items = ws.get("ground_items", [])
            self._state.controlled_entity_pos = ws["controlled_entity_pos"]
    ...
    def _start_combat(self, target: Entity, ambush: bool = False) -> None:
        ...
        combatants = [self._state.controlled_entity]
    ...
    def _handle_domain_event(self, event) -> None:
        ...
        if event.name == "ReactionRequired":
            ...
            item = next(
                (candidate for candidate in self._state.controlled_entity.inventory
                 if candidate.name == event.payload["item_name"]),
                None,
            )
            if item is not None:
                self._on_two_hand_equip(item, hand=event.payload["hand"])
            ...
            if item is not None:
                self._on_torch_action(item, event.payload["mode"])
    ...
    def _next_turn(self, *, reset_combat_ticked: bool = True) -> None:
        ...
        hostiles = [e for e in alive if are_hostile(e, self._state.controlled_entity) and not e.is_dead]
        if not hostiles:
