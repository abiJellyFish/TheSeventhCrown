...
class InventoryMixin:
    ...
    def _use_item(self, cmd: str) -> None:
        ...
        try:
            idx = int(cmd[1:]) - 1
            inv = self._state.controlled_entity.inventory
            if 0 <= idx < len(inv):
                item = inv[idx]
        ...
        if is_two_handed:
            ...
            for hand in ("left_hand", "right_hand"):
                ...
                if old is not None:
                    ...
                    equip[hand] = None
                    _add_to_inventory(self._state.controlled_entity, old)
            ...
            if equip.get(hand) is None:
                ...
                if other is not None:
                    ...
                    if 'two_handed' in other_props:
                        ...
                        _add_to_inventory(self._state.controlled_entity, other)
    ...
    def _extinguish_torch_if_lit(self, item) -> None:
        ...
        if ls.condition == "lit":
            ls.condition = "unlit"
            self._state.unregister_light(self._state.controlled_entity_pos)
    ...
    UNEQUIP_SLOTS = {
        ...
        3: "head", 4: "chest", 5: "arms", 6: "legs", 7: "spellbook",
    }
    ...
    def _handle_unequip(self, cmd: str) -> None:
        ...
        try:
            ...
            if slot is None:
                self._act_log.add("用法: :U1 左手 :U2 右手 :U3 头部 :U4 躯干 :U5 双臂 :U6 双腿 :U7 法术书")
                return
    ...
    def _handle_item_menu_select(self, num: int, top: dict) -> None:
        ...
        if action_label == "种植":
            self._action_plant(item, inv_index)
            stack.clear()
            self._close_input()
            self.refresh_all()
            return
        ...
        if action_label == "倒水":
            self._action_pour_water(item, inv_index)
            stack.clear()
            self._close_input()
            self.refresh_all()
            return
        ...
        if action_label == "取水":
            self._action_fetch_water(item, inv_index)
            stack.clear()
            self._close_input()
            self.refresh_all()
            return
        ...
        if action_label == "投掷":
            ...
            if throw_str_req > 0 and self._state.controlled_entity.stat("str") < throw_str_req:
                self._act_log.add("力量不足，无法投掷此物品")
                return
            # 进入投掷瞄准模式
            throw_range = get_throw_range(item, self._state.controlled_entity.vision_range)
    ...
    def _action_equip_left(self, item, inv_index: int) -> None:
        single = self._take_one_from_stack(item, self._state.controlled_entity.inventory, inv_index)
        if single:
    ...
    def _action_equip_spellbook(self, item, inv_index: int) -> None:
        """持有法术书：装备到法术书栏，旧书卸除回背包。"""
        single = self._take_one_from_stack(item, self._state.controlled_entity.inventory, inv_index)
        if single:
            self._equip_spellbook_from_inventory(single)

    def _action_cast_scroll(self, item, inv_index: int) -> None:
        """施放卷轴：复用法术瞄准和结算，但不要求法术天赋或法术位。"""
        spell_name = getattr(item, "spell", "") or item.name
        spell = {
            "name": spell_name,
    ...
    def _action_equip_accessory(self, item, inv_index: int) -> None:
        single = self._take_one_from_stack(item, self._state.controlled_entity.inventory, inv_index)
        if single:
            self._state.controlled_entity.add_accessory(single)
            self._act_log.add(f"{self._pn} 穿戴了 {single.name}")

    def _equip_spellbook_from_inventory(self, book) -> None:
        """装备法术书到法术书栏。已持有旧书则卸除回背包（不清理已记忆法术）。"""
        p = self._state.controlled_entity
        equip = p.equipment
        old = equip.get("spellbook")
        if old is not None:
            _add_to_inventory(p, old)
        equip["spellbook"] = book
        self._act_log.add(f"{self._pn} 持有了 {book.name}")
    ...
    def _on_torch_action(self, item, mode: str) -> None:
        ...
        if mode == "torch_ignite":
            self._state.register_light(self._state.controlled_entity_pos, radius, level)
            ls.condition = "lit"
    ...
    def _action_plant(self, item, inv_index: int) -> None:
        """种子种植：进入相邻格瞄准（1 钟摆）。"""
        self._state.pending_attack = {
            "mode": "plant",
            "seed_name": item.name,
    ...
    def _action_pour_water(self, item, inv_index: int) -> None:
        """水瓶倒水：相邻格瞄准，目标变潮湿，水瓶变空玻璃瓶。"""
        self._state.pending_attack = {
            "mode": "pour_water",
            "item": item,
            "inv_index": inv_index,
    ...
    def _action_fetch_water(self, item, inv_index: int) -> None:
        """空玻璃瓶取水：相邻水域瞄准，装水后变一瓶水。"""
        self._state.pending_attack = {
            "mode": "fetch_water",
            "item": item,
            "inv_index": inv_index,
    ...
    def _action_read(self, item, inv_index: int) -> None:
        """阅读文本物品（长老的提示等），日志显示 read_text。"""
        text = getattr(item, 'read_text', '') or item.description
        if text:
    ...
    _ITEM_ACTION_HANDLERS = {
        ...
        "持有": _action_equip_spellbook,
        "施法": _action_cast_scroll,
        "穿戴": _action_equip_accessory,
